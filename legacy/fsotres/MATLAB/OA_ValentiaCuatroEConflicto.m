function varargout = OA_ValentiaCuatroEConflicto(varargin)
% OA_VALENTIACUATROECONFLICTO MATLAB code for OA_ValentiaCuatroEConflicto.fig
%      OA_VALENTIACUATROECONFLICTO, by itself, creates a new OA_VALENTIACUATROECONFLICTO or raises the existing
%      singleton*.
%
%      H = OA_VALENTIACUATROECONFLICTO returns the handle to a new OA_VALENTIACUATROECONFLICTO or the handle to
%      the existing singleton*.
%
%      OA_VALENTIACUATROECONFLICTO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIACUATROECONFLICTO.M with the given input arguments.
%
%      OA_VALENTIACUATROECONFLICTO('Property','Value',...) creates a new OA_VALENTIACUATROECONFLICTO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaCuatroEConflicto_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaCuatroEConflicto_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaCuatroEConflicto

% Last Modified by GUIDE v2.5 07-Apr-2017 16:02:01

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @OA_ValentiaCuatroEConflicto_OpeningFcn, ...
    'gui_OutputFcn',  @OA_ValentiaCuatroEConflicto_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaCuatroEConflicto is made visible.
function OA_ValentiaCuatroEConflicto_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaCuatroEConflicto (see VARARGIN)

% Choose default command line output for OA_ValentiaCuatroEConflicto
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaCuatroEConflicto wait for user response (see UIRESUME)
...
set(handles.edit24,'String','0'); %cuenta de ensayos donde la rata cruzo
...
function varargout = OA_ValentiaCuatroEConflicto_OutputFcn(hObject, eventdata, handles)
...
Conflicto=str2num(get(handles.edit24,'String'));
...
[Secuencia]=OA_SecuenciaEnsayosConflicto(NumRepLado,Riesgo,Conflicto);
%Creamos una nueva funcion para obtener la secuencia (OA_SecuenciaEnsayosConflicto), ahora tendremos:
%0: ensayo seguro
%1: ensayo de conflicto, antes denominado de riesgo
%2: ensayo de peligroso (3) en donde aparecen estimulo sonoro y electrico
%pero no la luz.
...
%             end
        ...
        if((Secuencia(Ensayo,2)==1)|(Secuencia(Ensayo,2)==3)) %ensayo con riesgo o desde marzo 22 del 2017 ensayo conflicto
            ...
            if(get(handles.checkbox8,'Value')==1)
                ...
                OA_ValentiaEstimuloI(handles.OA,2,0); %dejamos sonido apagado luz prendida
            end
            OA_ValentiaElectrico(handles.OA,1);
            if(get(handles.checkbox7,'Value')==1)   
                if(Secuencia(Ensayo,2)==1) %solo si es ensayo conflicto
                    OA_ValentiaEstimuloI(handles.OA,2,1); %dejamos sonido apagado luz prendida
                end    
            end
            if(get(handles.checkbox10,'Value')==1)
                OA_ValentiaEstimuloI(handles.OA,2,2); %dejamos sonido apagado luz intermitente
            end
            ES=2;
            EL=2;
...
%         end
        ...
        OA_ValentiaFinBuscaIzquierda(handles.OA);
...
%             end
        ...
        if((Secuencia(Ensayo,2)==1)||(Secuencia(Ensayo,2)==3)) %ensayo con  riesgo
            if(SonidoIniR==1) %sonido inicial
                OA_Sonidos(handles.GS,.1,0,0,1000,1);
                pause(.5);
            end
            ...
            if(get(handles.checkbox8,'Value')==1)
                ...
                OA_ValentiaEstimuloI(handles.OA,2,0); %prendemos led rojo
            end
            OA_ValentiaElectrico(handles.OA,1);
            ...
            if(get(handles.checkbox10,'Value')==1)
                OA_ValentiaEstimuloD(handles.OA,0,2); %dejamos sonido apagado luz prendida
            end
            PelletsEvento=str2num(get(handles.edit17,'String'));
%             if(get(handles.checkbox9,'Value')==1)
%                 OA_ValentiaPalanca(handles.OA,'D',1); %nos aseguramos que la palanca der este afuera
...
%             end
        OA_ValentiaResetPalancas(handles.OA);
            ...
            while(1)
                DDA=0;
                DD=0;
                [DI,DD,DatosP]=OA_ValentiaRevisaPalanca(handles.OA);
                B=[DD DDA]
                ...
                if((DD>=PalXRec)&& EnsayoMismoLado==1 )
                    if(toc(R1)>(TultimaPalanca+IntVar+0.2)) %verificamos que la respuesta se presente despues de un retardo
                        TultimaPalanca=toc(R1) %guardamos el tiempo de la ultima palanca
                        Resultados=[Resultados;[Ensayo 0 Secuencia(Ensayo,2) toc(R2) toc(R0) ContadorTI ContadorTD LatMotDer]];
                        set(handles.uitable1,'Data',Resultados);
                        for iR=1:PelletsEvento
