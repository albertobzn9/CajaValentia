function varargout = OA_ValentiaCuatroF(varargin)
% OA_VALENTIACUATROF MATLAB code for OA_ValentiaCuatroF.fig
%      OA_VALENTIACUATROF, by itself, creates a new OA_VALENTIACUATROF or raises the existing
%      singleton*.
%
%      H = OA_VALENTIACUATROF returns the handle to a new OA_VALENTIACUATROF or the handle to
%      the existing singleton*.
%
%      OA_VALENTIACUATROF('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIACUATROF.M with the given input arguments.
%
%      OA_VALENTIACUATROF('Property','Value',...) creates a new OA_VALENTIACUATROF or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaCuatroF_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaCuatroF_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaCuatroF

% Last Modified by GUIDE v2.5 09-Apr-2014 12:00:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @OA_ValentiaCuatroF_OpeningFcn, ...
    'gui_OutputFcn',  @OA_ValentiaCuatroF_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaCuatroF is made visible.
function OA_ValentiaCuatroF_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaCuatroF (see VARARGIN)

% Choose default command line output for OA_ValentiaCuatroF
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaCuatroF wait for user response (see UIRESUME)
...
function varargout = OA_ValentiaCuatroF_OutputFcn(hObject, eventdata, handles)
...
%             end
         ...
         OA_ValentiaResetPalancas(handles.OA);
            ...
            while(1)
                ...
                if((DI>=PalXRec)&& EnsayoMismoLado==0 && (DIA~=DI)) %%si no se repite el mismo lado
                ...
                if((DI>=PalXRec)&& EnsayoMismoLado==1 && (DIA~=DI)) %%si se repite el mismo lado
