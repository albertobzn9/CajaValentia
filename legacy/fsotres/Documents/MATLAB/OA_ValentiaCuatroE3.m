function varargout = OA_ValentiaCuatroE3(varargin)
%Cambios solicitados por Ely para agregar ensayos neutros
% OA_VALENTIACUATROE3 MATLAB code for OA_ValentiaCuatroE3.fig
%      OA_VALENTIACUATROE3, by itself, creates a new OA_VALENTIACUATROE3 or raises the existing
%      singleton*.
%
%      H = OA_VALENTIACUATROE3 returns the handle to a new OA_VALENTIACUATROE3 or the handle to
%      the existing singleton*.
%
%      OA_VALENTIACUATROE3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIACUATROE3.M with the given input arguments.
%
%      OA_VALENTIACUATROE3('Property','Value',...) creates a new OA_VALENTIACUATROE3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaCuatroE3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaCuatroE3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaCuatroE3

% Last Modified by GUIDE v2.5 14-Oct-2015 18:40:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @OA_ValentiaCuatroE3_OpeningFcn, ...
    'gui_OutputFcn',  @OA_ValentiaCuatroE3_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaCuatroE3 is made visible.
function OA_ValentiaCuatroE3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaCuatroE3 (see VARARGIN)

% Choose default command line output for OA_ValentiaCuatroE3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaCuatroE3 wait for user response (see UIRESUME)
...
function varargout = OA_ValentiaCuatroE3_OutputFcn(hObject, eventdata, handles)
...
Neutros=str2num(get(handles.edit24,'String'));
...
[Secuencia]=OA_SecuenciaEnsayos5(NumRepLado,Riesgo,Neutros);
...
%             end
        ...
        if(Secuencia(Ensayo,2)==3) %ensayo neutro
%             if(SonidoIniS==1) %sonido inicial
%                 OA_Sonidos(handles.GS,.1,1000,1,0,0);
%                 pause(.5);
%             end
...
%             EL=1;
             PelletsEvento=str2num(get(handles.edit3,'String'));
            DurMaxEns=str2num(get(handles.edit25,'String'));
%             if(get(handles.checkbox6,'Value')==1)
%                 OA_ValentiaPalanca(handles.OA,'I',1); %nos aseguramos que la palanca izq este afuera
%             end
        end
            ...
            while(1)
                ...
                if((DI>=PalXRec)&& EnsayoMismoLado==0 ) %%si no se repite el mismo lado
                    ...
                    for iR=1:PelletsEvento
                        'recom'
                        if(Secuencia(Ensayo,2)<3) %se agrega para evitar los ensayos neutros
