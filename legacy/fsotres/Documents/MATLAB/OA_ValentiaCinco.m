function varargout = OA_ValentiaCinco(varargin)
% OA_VALENTIACINCO MATLAB code for OA_ValentiaCinco.fig
%      OA_VALENTIACINCO, by itself, creates a new OA_VALENTIACINCO or raises the existing
%      singleton*.
%
%      H = OA_VALENTIACINCO returns the handle to a new OA_VALENTIACINCO or the handle to
%      the existing singleton*.
%
%      OA_VALENTIACINCO('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIACINCO.M with the given input arguments.
%
%      OA_VALENTIACINCO('Property','Value',...) creates a new OA_VALENTIACINCO or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaCinco_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaCinco_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaCinco

% Last Modified by GUIDE v2.5 14-May-2013 15:52:31

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OA_ValentiaCinco_OpeningFcn, ...
                   'gui_OutputFcn',  @OA_ValentiaCinco_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaCinco is made visible.
function OA_ValentiaCinco_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaCinco (see VARARGIN)

% Choose default command line output for OA_ValentiaCinco
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaCinco wait for user response (see UIRESUME)
...
function varargout = OA_ValentiaCinco_OutputFcn(hObject, eventdata, handles) 
...
[Secuencia]=OA_SecuenciaLargasEnsayos(TipoSecuencia,Riesgo,1);
end
if(get(handles.checkbox3,'Value')==0)
[Secuencia]=OA_SecuenciaLargasEnsayos(TipoSecuencia,Riesgo,0);
end
    ...
    if(strcmp(Lado,'D')==1)
       ...
       if(SonidoIni==1) %sonido inicial
        ...
        if(Secuencia(Ensayo,2)==0) %ensayo seguro
           ...
           % OA_Sonidos(handles.GS,DurMaxEns,freqAI,AmpAI,0,0); %mandamos tono izq
            ES=0;
            EL=1;
        end
     ...
     if(strcmp(Lado,'I')==1) 
        ...
        if(Secuencia(Ensayo,2)==0) %ensayo seguro
          ...
          % OA_Sonidos(handles.GS,DurMaxEns,0,0,freqAD,AmpAD); %mandamos tono der
        end
        if(Secuencia(Ensayo,2)==1) %ensayo con  riesgo
