function varargout = OA_ValentiaUnoCP(varargin)
% OA_VALENTIAUNOCP MATLAB code for OA_ValentiaUnoCP.fig
%      OA_VALENTIAUNOCP, by itself, creates a new OA_VALENTIAUNOCP or raises the existing
%      singleton*.
%
%      H = OA_VALENTIAUNOCP returns the handle to a new OA_VALENTIAUNOCP or the handle to
%      the existing singleton*.
%
%      OA_VALENTIAUNOCP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIAUNOCP.M with the given input arguments.
%
%      OA_VALENTIAUNOCP('Property','Value',...) creates a new OA_VALENTIAUNOCP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaUnoCP_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaUnoCP_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaUnoCP

% Last Modified by GUIDE v2.5 30-Jan-2013 13:57:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OA_ValentiaUnoCP_OpeningFcn, ...
                   'gui_OutputFcn',  @OA_ValentiaUnoCP_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaUnoCP is made visible.
function OA_ValentiaUnoCP_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaUnoCP (see VARARGIN)

% Choose default command line output for OA_ValentiaUnoCP
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaUnoCP wait for user response (see UIRESUME)
...
function varargout = OA_ValentiaUnoCP_OutputFcn(hObject, eventdata, handles) 
        ...
        OA_ValentiaResetPalancas(handles.OA);
         ...
         while(1)
             ...
             if((DI>=PalXRec)&& EnsayoMismoLado==0) %%si no se repite el mismo lado
                ...
                Resultados=[Resultados;[Ensayo 1 Secuencia(Ensayo,2) toc(R2) toc(R0) ContadorTI ContadorTD]]; 
        ...
        if(Secuencia(Ensayo,2)==1)
         ...
         while(1)
             ...
             if((DD>=PalXRec)&& EnsayoMismoLado==0)
                TultimaPalanca=toc(R1) %guardamos el tiempo de la ultima palanca 
                Resultados=[Resultados;[Ensayo 0 Secuencia(Ensayo,2) toc(R2) toc(R0) ContadorTI ContadorTD]]; 
                set(handles.uitable1,'Data',Resultados); 
                load('PelletsEvento','PelletsEvento');
                for iR=1:PelletsEvento
