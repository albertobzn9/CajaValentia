function varargout = OA_ValentiaCuatroA(varargin)
% OA_VALENTIACUATROA MATLAB code for OA_ValentiaCuatroA.fig
%      OA_VALENTIACUATROA, by itself, creates a new OA_VALENTIACUATROA or raises the existing
%      singleton*.
%
%      H = OA_VALENTIACUATROA returns the handle to a new OA_VALENTIACUATROA or the handle to
%      the existing singleton*.
%
%      OA_VALENTIACUATROA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in OA_VALENTIACUATROA.M with the given input arguments.
%
%      OA_VALENTIACUATROA('Property','Value',...) creates a new OA_VALENTIACUATROA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OA_ValentiaCuatroA_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OA_ValentiaCuatroA_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OA_ValentiaCuatroA

% Last Modified by GUIDE v2.5 19-Aug-2013 13:09:49

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OA_ValentiaCuatroA_OpeningFcn, ...
                   'gui_OutputFcn',  @OA_ValentiaCuatroA_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OA_ValentiaCuatroA is made visible.
function OA_ValentiaCuatroA_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OA_ValentiaCuatroA (see VARARGIN)

% Choose default command line output for OA_ValentiaCuatroA
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes OA_ValentiaCuatroA wait for user response (see UIRESUME)
...
function varargout = OA_ValentiaCuatroA_OutputFcn(hObject, eventdata, handles) 
       ...
       if(SonidoIni==1) %sonido inicial
        ...
        OA_ValentiaResetPalancas(handles.OA);
         ...
         while(1)
          ...
          if((get(handles.checkbox6,'Value')==1)&&(Secuencia(Ensayo,1)~=Secuencia(Ensayo+1,1))) %si se pide meter la palanca y el ensayo siguiente es de lado diferente
              OA_ValentiaPalanca(handles.OA,'I',2); %nos aseguramos que la palanca der este afuera 
          end    
          Ensayo=Ensayo+1; 
