...
set(handles.text10, 'Position', posicionesExperimentales.relojEtiqueta, ...
    'HorizontalAlignment', 'center', 'String', 'Reloj ensayo (s)');
...
handles.AvisoLedFinal = uicontrol('Parent', hObject, ...
    ...
    'TooltipString', ['Al terminar la habituacion final, enciende el LED ', ...
        'marcador 100 ms cada 1 s hasta cerrar Guardar resultados.']);
handles.ContadorHabituacion = uicontrol('Parent', hObject, ...
    'Style', 'text', ...
    'String', 'Hab.: --', ...
    'Tag', 'ContadorHabituacion', ...
    'Units', 'characters', ...
    'Position', posicionesExperimentales.contadorHabituacion, ...
    'FontSize', 8, ...
    'HorizontalAlignment', 'left', ...
    'TooltipString', 'Tiempo transcurrido y total de la habituacion activa.');
...
cmc_actualizar_contador_habituacion(handles.ContadorHabituacion, ...
    'inicial', 0, THabitua);
while toc(RHabituacion)<THabitua
    ...
    cmc_actualizar_contador_habituacion(handles.ContadorHabituacion, ...
        'inicial', toc(RHabituacion), THabitua);
    pause(min(0.3,max(0,THabitua-toc(RHabituacion))));
end
cmc_actualizar_contador_habituacion(handles.ContadorHabituacion, ...
    'inicial', THabitua, THabitua);
...
while toc(RHabituacion)<THabitua
    ...
    cmc_actualizar_reloj_fase(handles.edit9,'Habituacion final (s)', ...
        toc(RHabituacion),THabitua);
    cmc_actualizar_contador_habituacion(handles.ContadorHabituacion, ...
        'final', toc(RHabituacion), THabitua);
    pause(min(0.3,max(0,THabitua-toc(RHabituacion))));
end
cmc_actualizar_contador_habituacion(handles.ContadorHabituacion, ...
    'final', THabitua, THabitua);
cmc_actualizar_reloj_fase(handles.edit9,'Reloj de duracion del ensayo (s)',0,[]);
